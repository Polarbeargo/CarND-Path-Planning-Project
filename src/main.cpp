...
int main()
{
	...
	// Add start in lane 1 (0 is left, 1 is middle, 2 is right)
	int lane = 1;

	// Set reverence velocity to target vehicle MPH)
	double rev_vel = 0.0;

	h.onMessage([&rev_vel, &lane, &map_waypoints_x, &map_waypoints_y, &map_waypoints_s, &map_waypoints_dx, &map_waypoints_dy](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
																																																														uWS::OpCode opCode) {
		...
		if (length && length > 2 && data[0] == '4' && data[1] == '2')
		{
			...
			if (s != "")
			{
				...
				if (event == "telemetry")
				{
					...
					// Add Fernet evenly evry 30m space point ahead of the starting reference.
					...
					for (int i = 0; i < ptsx.size(); i++)
					{
						...
						// Shift car coordinates to 0 degree.
					...
					// Define the actual output path points from previous path for the planner.
					...
					// Calculate how to break up Spline point so that we travel our desire velocity.
					double target_x = 30.0;
					double target_y = s(target_x);
					double target_dist = sqrt(target_x * target_x + target_y * target_y);
					double x_add_on = 0;

					// Fill up the rest path planner after filling with previous point.
